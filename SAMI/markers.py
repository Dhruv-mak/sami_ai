...
def findmarkers(self,dict_clustermap, adata,adj_pval_cutoff,top,adata2=None, workdir=None):  
    ...
    # return os.path.join(workdir,f'{self.region}_pvalue.csv'), os.path.join(workdir,f'{self.region}_marker.csv')
    return f"{self.region}_pvalue.csv", f"{self.region}_marker.csv"
...
def circular_tree(self, clusters=None,top_n=5, show=True, workdir=None):
    ...
    plt.savefig(os.path.join(workdir,f'{self.region}_circulartree.png'))
    if show==False:
        plt.close()
    else:
        plt.show()
    return os.path.join(workdir,f'{self.region}_circulartree.png')
