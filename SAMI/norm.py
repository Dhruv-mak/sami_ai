...
def GroupMedianPQN(x):
    ...
    for group in unique_groups: # type: ignore
        group_indices = np.where(np.array(group_labels) == group)[0] # type: ignore
    ...
    for i, row in enumerate(factor):
        factor_dict[unique_groups[i]] = row # type: ignore
...
def Normalization(
    ...
    if rowNorm is not None and rowNorm not in [
        "SumNorm",
        "MedianNorm",
        "CompNorm",
        "SamplePQN",
        ...
        raise ValueError(
            "'rowNorm' should be None or one of those methods: 'SumNorm', 'MedianNorm', 'CompNorm', 'SamplePQN', 'GroupPQN', 'SpecNorm', 'GroupMedianPQN'."
        )
        ...
        if group_column is None:
            raise ValueError(
                "Please provide the name of the column indicating different groups."
            )
        else:
            ...
            print(
                "Probabilistic Quotient Normalization by a customized sample based on median of each group."
            )
    ...
    ### Transformation
    if transNorm is not None and transNorm not in [
        "LogTrans1",
        "LogTrans2",
        "SquareRootTrans",
        "CubeRootTrans",
        ...
        raise ValueError(
            "'transNorm' should be None or one of those methods: 'LogTrans', 'SquareRootTrans', 'CubeRootTrans'."
        )
    ...
    ### Scaling
    if scaleNorm is not None and scaleNorm not in [
        "MeanCenter",
        "AutoNorm",
        "ParetoNorm",
        "RangeNorm",
        ...
        raise ValueError(
            "'scaleNorm' should be None or one of those methods: 'MeanCenter', 'AutoNorm', 'ParetoNorm', 'RangeNorm'."
        )
        ...
        data = ParetoNorm(data) # type: ignore
...
def create_norm_dataset(data_path, pattern, output_path) -> str:
    """
    ...
    output_path : str
        Path to the directory where the normalized datasets will be saved.

    Returns
    -------
    str
        Message indicating the completion of the normalization process.
    ...
    for file in os.listdir(data_path):
        ...
        if re.match(pattern, file):
            print("-----------------------------")
            df = validate_and_process_csv(os.path.join(data_path, f"{file}"))
            if len(regions) == 0:
                regions.update(df["region"].unique().tolist())
            else:
                if not all(region in regions for region in df["region"].unique()):
                    print(
                        f"File {file} doesn't have same region as the previous files. Skipping..."
                    )
                    continue
            if df is None:
            ...
            name = f"{name}_norm"
            df_norm.to_csv(os.path.join(output_path, f"{name}{ext}"), index=False)
            output += (
                f'{os.path.join(output_path,f"{name}{ext}")} the new normalized csv.\n'
            )
    output += f"Normalization completed for {len(os.listdir(data_path))} files."
    output += f"\nThe available regions are {list(regions)}."
    return output
